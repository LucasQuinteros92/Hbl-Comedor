...
def escribeSeparador(log):
    rutaLogs = os.getcwd() + '/log/'
    if os.path.exists(rutaLogs) is False:
            os.mkdir(rutaLogs)
            os.chmod(rutaLogs,  stat.S_IROTH |
                                stat.S_IWOTH |
                                stat.S_IXOTH )
    logFile = open(rutaLogs + log, "a")
        ...
        if tamanioArchivo >= hbl.LOGS_tamanioRotator:
            ...
            with zipfile.ZipFile(archivo, mode='w') as zf: 
                ...
                if os.path.exists(origen):
                    if os.path.exists(destino) is False:
                        os.mkdir(destino)
                        os.chmod(destino,   stat.S_IROTH |
                                            stat.S_IWOTH |
                                            stat.S_IXOTH ) 
